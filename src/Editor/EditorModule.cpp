...
void EditorModule::RenderUI() {
    ...
    const bool detachedPreviewActive = m_DetachedPreviewActive;
    ...
    float nodesPanelTargetWidth = isNodeBrowserOpen ? std::min(nodeBrowserMaxWidth, workspaceSize.x) : 0.0f;
    ...
    if (revealedRightWidth > 1.0f) {
        ...
        m_Viewport.Render(this, m_LibraryLoadCanvasRevealAlpha, EditorViewport::HostMode::DockedPane);
        ImGui::EndChild();
...
void EditorModule::RenderDetachedPreviewWindow() {
    ...
    if (m_DetachedPreviewRequestFocus) {
    ...
    const bool previousNoAutoMerge = io.ConfigViewportsNoAutoMerge;
    ...
    ImGui::Begin("Preview", &keepOpen, flags);
    io.ConfigViewportsNoAutoMerge = previousNoAutoMerge;
    ImGuiViewport* detachedViewport = ImGui::GetWindowViewport();
    const bool ownsDedicatedViewport =
        detachedViewport != nullptr &&
        detachedViewport->ID != ImGui::GetMainViewport()->ID;
    if (ownsDedicatedViewport) {
        m_DetachedPreviewPlacementInitialized = true;
        if (GLFWwindow* platformWindow = static_cast<GLFWwindow*>(detachedViewport->PlatformHandle)) {
            NativeWindowTheme::Apply(platformWindow);
            if (m_DetachedPreviewRequestFocus) {
                glfwShowWindow(platformWindow);
                glfwFocusWindow(platformWindow);
                m_DetachedPreviewRequestFocus = false;
            }
        }
    }
