...
#include "Layers/ClassicalRgbDenoiseLayer.h"
...
const std::vector<LayerDescriptor>& Descriptors() {
    static const std::vector<LayerDescriptor> descriptors = {
        ...
        { LayerType::ToneCurve, "ToneCurve", "Tone Curve", "Tone Curve", "Color / Tone", "Combined scene-referred local baseline, adaptive tonal balancing, and point-curve finishing for RAW and HDR tone shaping.", {}, MakeLayer<ToneCurveLayer>, Hidden, FullImagePreferred, false, "Develop now owns the normal finish workflow; standalone Tone Curve remains internal / legacy-only.", "Scene output can exceed display range; add View Transform for final preview/export.", { "tone", "curve", "scene-referred", "raw", "advanced-ui", "full-image-preferred", "combined-tone", "hidden" } },
        ...
        { LayerType::ClassicalRgbDenoise, "ClassicalRgbDenoise", "Classical RGB Denoise", "Classical RGB Denoise", "Blur / Focus", "Offline classical RGB denoise sweep with candidate scoring, winner blending, and detail anchoring.", {}, MakeLayer<ClassicalRgbDenoiseLayer>, Experimental, FullImagePreferred, true, "CPU prototype: press Run Denoise to compute and cache a result from the incoming graph image.", "Full-image RGB cleanup; not intended for isolated scalar channel streams.", { "denoise", "classical", "advanced-ui", "full-image-preferred", "experimental" } },
