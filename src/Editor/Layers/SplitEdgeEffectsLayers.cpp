#include "SplitEdgeEffectsLayers.h"
...
SplitEdgeEffectsLayerBase::~SplitEdgeEffectsLayerBase() {
    if (m_ShaderProgram) {
        glDeleteProgram(m_ShaderProgram);
    }
}

void SplitEdgeEffectsLayerBase::InitializeGL() {
    m_ShaderProgram = GLHelpers::CreateShaderProgram(kEdgeVert, kEdgeFrag);
}

void SplitEdgeEffectsLayerBase::Execute(unsigned int inputTexture, int width, int height, FullscreenQuad& quad) {
    ...
    glUniform1i(glGetUniformLocation(m_ShaderProgram, "uMode"), GetMode());
...
void SplitEdgeEffectsLayerBase::RenderUI() {
...
json SplitEdgeEffectsLayerBase::Serialize() const {
    json j;
    j["type"] = GetTypeId();
    j["blend"] = m_Blend;
...
void SplitEdgeEffectsLayerBase::Deserialize(const json& j) {
