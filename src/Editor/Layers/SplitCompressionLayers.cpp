#include "SplitCompressionLayers.h"
...
SplitCompressionLayerBase::~SplitCompressionLayerBase() {
    if (m_ShaderProgram) {
        glDeleteProgram(m_ShaderProgram);
    }
}

void SplitCompressionLayerBase::InitializeGL() {
    m_ShaderProgram = GLHelpers::CreateShaderProgram(kCompressionVert, kCompressionFrag);
}

void SplitCompressionLayerBase::Execute(unsigned int inputTexture, int width, int height, FullscreenQuad& quad) {
    ...
    glUniform1i(glGetUniformLocation(m_ShaderProgram, "uMethod"), GetMethod());
...
void SplitCompressionLayerBase::RenderUI() {
    ...
    ImGui::TextDisabled("Repeated web passes are folded into the effect strength here.");
}

json SplitCompressionLayerBase::Serialize() const {
    json j;
    j["type"] = GetTypeId();
    j["quality"] = m_Quality;
...
void SplitCompressionLayerBase::Deserialize(const json& j) {
