#include "SplitDenoisingLayers.h"
...
SplitDenoiseLayerBase::~SplitDenoiseLayerBase() {
    if (m_ShaderProgram) {
        glDeleteProgram(m_ShaderProgram);
    }
}

void SplitDenoiseLayerBase::InitializeGL() {
    m_ShaderProgram = GLHelpers::CreateShaderProgram(kDenoiseVert, kDenoiseFrag);
}

void SplitDenoiseLayerBase::Execute(unsigned int inputTexture, int width, int height, FullscreenQuad& quad) {
...
void SplitDenoiseLayerBase::RenderUI() {
...
json SplitDenoiseLayerBase::Serialize() const {
    json j;
    j["type"] = GetTypeId();
...
void SplitDenoiseLayerBase::Deserialize(const json& j) {
